...
# 1. Попытка примонтировать SSD nctk (для Loki/Prometheus)
if [[ "$1" != "down" ]] && [[ "$1" != "stop" ]]; then
    if [[ ! -d /mnt/nctk-ssd/loki ]] || ! [[ -w /mnt/nctk-ssd/loki ]]; then
        echo "Попытка примонтировать SSD nctk..."
        bash scripts/mount-nctk-ssd-on-centos.sh 2>/dev/null || echo "SSD не примонтирован — используем локальные volumes"
    fi
fi

# 2. Установка COMPOSE_FILE (как в start-with-ssd)
DATA_PATH="${DOCKER_DATA_PATH:-/mnt/nctk-ssd}"
[[ -f .env ]] && DATA_PATH=$(grep -E '^DOCKER_DATA_PATH=' .env 2>/dev/null | cut -d= -f2-) || true
DATA_PATH="${DATA_PATH:-/mnt/nctk-ssd}"
