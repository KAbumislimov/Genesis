...
r = st.post('/api/ui-default', json={'style': 'carbon'}); show('staff без привилегии → 403', r.status_code == 403, r.status_code)
r = st.get('/api/ui-default'); show('staff GET → 403', r.status_code == 403)
r = ad.post('/api/ui-default', json={'style': 'carbon'}); show('админ выбирает «carbon»', r.status_code == 200 and r.get_json().get('current') == 'carbon', r.get_json())
...
row = q("SELECT value FROM settings WHERE key='ui_default'"); show('значение сохранено в settings', bool(row) and row[0] == 'carbon', row)
# у пользователя без своего выбора открывается основной, а личный выбор сильнее
r = hd.get('/'); show('helpdesk без своего выбора: «/» → /v17 (Карбон)', r.status_code == 302 and r.headers['Location'].endswith('/v17'), r.headers.get('Location'))
hd.set_cookie('ui', 'emerald'); r = hd.get('/'); show('личный выбор (emerald) сильнее основного', r.headers['Location'].endswith('/v10'), r.headers.get('Location'))
hd.delete_cookie('ui')
...
r = st.post('/api/ui-default', json={'style': 'ice'}); show('staff теперь может → 200', r.status_code == 200, r.status_code)
...
r = st.post('/api/ui-default', json={'style': 'night'}); show('после снятия staff снова 403', r.status_code == 403)
# предпросмотр не сохраняет выбор
c = client('helpdesk'); r = c.get('/v18?preview=1')
...
r = c.get('/v18'); show('обычный заход на /v18 запоминает выбор (Set-Cookie ui=night)', 'ui=night' in (r.headers.get('Set-Cookie') or ''))
# превью-картинки: только для вошедших, без обхода каталога
r = ad.get('/ui-preview/emerald.jpg'); show('превью emerald.jpg вошедшему → 200 image/jpeg', r.status_code == 200 and r.mimetype == 'image/jpeg', (r.status_code, r.mimetype))
r = anon.get('/ui-preview/emerald.jpg'); show('без входа → редирект/401 (не 200)', r.status_code in (301, 302, 401, 403), r.status_code)
...
r = ad.get('/static/ui_previews/emerald.jpg'); show('в /static превью не лежат', r.status_code == 404)
