#!/bin/bash
# Настройка SSD 512 ГБ на nctk (10.20.0.41) + NFS export
# Запуск: sudo bash setup-ssd-on-nctk.sh /dev/sdb    — диск целиком (пересоздаст раздел)
#         sudo bash setup-ssd-on-nctk.sh /dev/sdb1   — только раздел (NTFS→ext4, данные удалятся)
