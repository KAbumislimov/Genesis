#!/bin/bash
# Запуск стека logs. Если SSD на nctk примонтирован — использует его, иначе — локальные volumes.
...
if [[ -d "$DATA_PATH/loki" ]] && [[ -w "$DATA_PATH/loki" ]]; then
    ...
    echo "SSD не примонтирован — используем локальные volumes. Для SSD: bash scripts/mount-nctk-ssd-on-centos.sh"
