#!/bin/bash
# Синхронизация времени nctk с сервера — запускается с 10.10.4.120 по cron
SSH_KEY="/home/kamran/.ssh/campus_bot"
NCTK="nctk@10.20.0.41"
LOG="/home/kamran/log/nctk-timesync.log"
mkdir -p "$(dirname "$LOG")"

# Локальное время сервера — timedatectl set-time ожидает LOCAL, не UTC
STIME=$(date '+%Y-%m-%d %H:%M:%S')

ssh -i "$SSH_KEY" -o BatchMode=yes -o ConnectTimeout=8 "$NCTK" "
...
STATUS=$?
sleep 2
NCTK_TIME=$(ssh -i "$SSH_KEY" -o BatchMode=yes -o ConnectTimeout=5 "$NCTK" "date '+%Y-%m-%d %H:%M:%S'" 2>/dev/null)
SERVER_TIME=$(date '+%Y-%m-%d %H:%M:%S')
echo "[$(date '+%F %T')] server=$SERVER_TIME nctk=$NCTK_TIME exit=$STATUS" >> "$LOG"
