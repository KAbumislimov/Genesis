#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Telegram-бот Клиент 1: тот же интерфейс что client2/client1, воспроизведение по SSH на клиент."""
import os
import sys
import json
import html
...
def load_config():
    ...
    env.setdefault('CLIENT2_HOST', os.environ.get('CLIENT2_HOST', ''))
    env.setdefault('CLIENT1_HOST', os.environ.get('CLIENT1_HOST', ''))
    for k in ('CLIENT_HOST', 'CLIENT_USER', 'CLIENT_INBOX', 'SSH_KEY', 'DEFAULT_VOL',
              'SERVER_HOST', 'CLIENT2_HOST', 'CLIENT1_HOST', 'LOG_GROUP_ID', 'LOG_GROUP_IDS'):
        if os.environ.get(k):
...
def get_time_all_machines(update_date_ts=None):
    """Время на Server, client2, client1, Телефон. Бот работает на сервере."""
    cfg = load_config()
    key = cfg.get('SSH_KEY') or cfg.get('SSH_KEY_FOR_TIME')
    srv = cfg.get('SERVER_HOST', '10.10.4.120')
    client2_host = cfg.get('CLIENT2_HOST') or (cfg.get('CLIENT_HOST') if cfg.get('CLIENT_USER') == 'client2' else '')
    client1_host = cfg.get('CLIENT1_HOST') or (cfg.get('CLIENT_HOST') if cfg.get('CLIENT_USER') == 'client1' else '')
    lines = []
    lines.append(f"🖥 Server (centos): {datetime.now().strftime('%H:%M:%S %d.%m.%Y')}")
    if client1_host:
        lines.append(f"🖥 client1: {get_time_remote(client1_host, 'client1', key)}")
    if client2_host:
        lines.append(f"🖥 client2: {get_time_remote(client2_host, 'client2', key)}")
    if update_date_ts:
        try:
...
def get_server_status():
    ...
    client1_host = cfg.get('CLIENT1_HOST') or (cfg.get('CLIENT_HOST') if cfg.get('CLIENT_USER') == 'client1' else '')
    if client1_host:
        out = _ssh_sysinfo(client1_host, 'client1', key, sysinfo_cmd)
        results.append(_format_machine_status(f"client1 ({client1_host})", out))
    client2_host = cfg.get('CLIENT2_HOST') or (cfg.get('CLIENT_HOST') if cfg.get('CLIENT_USER') == 'client2' else '')
    if client2_host:
        out = _ssh_sysinfo(client2_host, 'client2', key, sysinfo_cmd)
        results.append(_format_machine_status(f"client2 ({client2_host})", out))
...
def _vol_change(cfg, direction):
    ...
    user = cfg.get('CLIENT_USER', 'client2')
    return True, (
        ...
        for line in raw:
            ...
            if m:
                ...
                if '|' in rest:
                    ...
                    track = track.strip()
                if 'Media' in rest:
                    out.append(f"📅 {date_part} {time_part[:5]} — Media: {rest.split('|')[-1].strip() if '|' in rest else rest[:50]}")
...
def get_next_on_schedule_text():
    ...
    lines = []
    for h, m, slot in CRON_SCHEDULE_MEDIA:
        if slot == "stop":
            continue
        event_min = h * 60 + m
        if event_min > cur_min:
            time_str = f"{h:02d}:{m:02d}"
            label = _media_slot_label(slot)
...
def run_recover():
    ...
    for svc in ("loki-tunnel-client1", "loki-tunnel-client2"):
        try:
...
def info_text():
    ...
    return f"""ℹ️ <b>Полная информация — бот Клиент 1</b>
...
def handle_update(token, chat_id, log_group_id, update):
    ...
    # Нормализуем команды с @botname: /start@client21_bot -> /start
...
def main():
    ...
    if not token:
        print('Задайте BOT_TOKEN в config.env или client1.env', file=sys.stderr)
        sys.exit(1)
