# Настройка SSD 512 ГБ на nctk (10.20.0.41) для хранения данных Loki/Prometheus

Сервер CentOS (10.10.4.120) переполнен.  
SSD на nctk используется через **SSHFS** (не требует интернета на nctk, не требует NFS).

## Схема

```
nctk (10.20.0.41)                    CentOS (10.10.4.120)
┌─────────────────────┐              ┌─────────────────────────┐
...
## Шаг 1: На nctk (10.20.0.41) — SSD и NFS
...
При **SSHFS** контейнеры пишут от имени пользователя nctk (не 10001/65534), поэтому нужны права 777:
...
Если Loki/Prometheus уже падают с `permission denied`, выполните на nctk:

```bash
# На nctk или с CentOS:
ssh nctk@10.20.0.41 'sudo chmod 777 /mnt/campus-data/loki /mnt/campus-data/prometheus'
```

Либо запустите скрипт: `ssh nctk@10.20.0.41 'sudo bash -s' < scripts/fix-ssd-permissions-on-nctk.sh`

### 1.5 NFS не нужен

При использовании SSHFS на nctk **не нужны** ни интернет, ни nfs-kernel-server.  
...
bash scripts/mount-nctk-ssd-on-centos.sh
```

SSH-ключ kamran → nctk должен быть без пароля (или используйте ssh-agent).

### 2.3 Включить использование SSD в campus-infra

```bash
cd /home/kamran/campus-infra
echo 'DOCKER_DATA_PATH=/mnt/nctk-ssd' >> .env
```
...
docker run --rm -v campus-infra_loki_data:/from -v /mnt/nctk-ssd/loki:/to alpine cp -a /from/. /to/
docker run --rm -v campus-infra_prometheus_data:/from -v /mnt/nctk-ssd/prometheus:/to alpine cp -a /from/. /to/
...
- При перезагрузке nctk CentOS не сможет писать на SSHFS до его включения

## Синхронизация логов (nctk, vm1)

Чтобы в Grafana отображались логи с nctk и vm1, запускайте периодически:
...
Требуется SSH-доступ без пароля: `ssh nctk@10.20.0.41` и `ssh vm1@10.70.0.41`. Ключ по умолчанию: `~/.ssh/campus_bot` (или задайте `SSH_KEY` в окружении).

## Node exporter на клиентах (nctk, vm1)

Метрики CPU/RAM/Disk приходят с node_exporter. Установите его на nctk и vm1:
