#!/bin/bash
# Установка wctk-клиента с нуля (Ubuntu 22.04).
...
WCTK_USER="${WCTK_USER:-wctk}"
WCTK_HOME="/home/$WCTK_USER"
BOT_DIR="$WCTK_HOME/telegram-campus-bot"
MEDIA_DIR="$WCTK_HOME/Media"

echo ""
echo "╔══════════════════════════════════════════╗"
echo "║       WCTK CLIENT INSTALL                 ║"
echo "╚══════════════════════════════════════════╝"
echo "  Пользователь: $WCTK_USER"
...
sudo chown "$WCTK_USER:audio" /run/campus-player
sudo chmod 2775 /run/campus-player

# tmpfiles.d — пересоздавать после перезагрузки
echo "d /run/campus-player 2775 $WCTK_USER audio -" | \
...
if [[ ! -f "/usr/local/bin/campus-playerctl" ]]; then
    sudo ln -sf "$WCTK_HOME/campus-playerctl" /usr/local/bin/campus-playerctl
fi
...
# campus-mpv — user-сервис (запускается под wctk)
SYSTEMD_USER_DIR="$WCTK_HOME/.config/systemd/user"
...
sudo loginctl enable-linger "$WCTK_USER"

# Запускаем user-сервис
sudo -u "$WCTK_USER" XDG_RUNTIME_DIR="/run/user/$(id -u "$WCTK_USER")" \
...
mkdir -p "$WCTK_HOME/Media"

CRON_TMP="/tmp/campus_wctk_cron_$$"
crontab -u "$WCTK_USER" -l 2>/dev/null > "$CRON_TMP" || true

# Проверяем не установлен ли уже
if ! grep -q "campus-cron-media-local" "$CRON_TMP" 2>/dev/null; then
    cat "$SCRIPT_DIR/crontab" >> "$CRON_TMP"
    crontab -u "$WCTK_USER" "$CRON_TMP"
...
# ─────────────────────────────────────────────
if [[ -f "$WCTK_HOME/bin/promtail" ]]; then
    cp -f "$SCRIPT_DIR/promtail-config.yaml" "$WCTK_HOME/promtail/config.yaml" 2>/dev/null || true
...
mkdir -p "$WCTK_HOME/campus-monitoring"
cp -f "$SCRIPT_DIR/campus-monitoring/audio-analyzer.py" "$WCTK_HOME/campus-monitoring/audio-analyzer.py"
mkdir -p "$SYSTEMD_USER_DIR"
...
echo ""
echo "╔══════════════════════════════════════════╗"
echo "║       WCTK УСТАНОВКА ЗАВЕРШЕНА ✅         ║"
echo "╚══════════════════════════════════════════╝"
echo ""
echo "  Статус mpv:  sudo -u wctk XDG_RUNTIME_DIR=/run/user/\$(id -u wctk) systemctl --user status campus-mpv"
...
echo "    Cockpit (опционально) — скопируй scripts/restore-client2.sh как scripts/restore-wctk.sh и поправь под эту машину."
echo ""
