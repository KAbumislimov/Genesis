#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Telegram-бот для клиента nctk: кнопки, список треков, логи в группу «наримановскую», /info."""
import os
import sys
import json
import re
...
def load_config():
    ...
    env.setdefault('DIR_KAMRAN', '/home/nctk/Landau/Kamran Music')
    env.setdefault('SERVER_HOST', '10.10.4.120')
    env.setdefault('VM1_HOST', '10.70.0.41')
...
def get_tracks_total():
    d = load_config().get('DIR_KAMRAN', '/home/nctk/Landau/Kamran Music')
...
def log_action(token, log_group_id, msg, also_file=True, chat_id=None):
    ...
    Формат: [ЧЧ:ММ:СС] [nctk] действие | @user"""
...
def get_server_status():
    ...
    results.append(_format_machine_status("nctk (локально)", local_out or "Ошибка"))
    srv = cfg.get('SERVER_HOST', '10.10.4.120')
    if srv:
...
def main():
    ...
    if log_group_id:
        try:
            send(token, log_group_id, f"🤖 [nctk] Бот запущен {datetime.now().strftime('%H:%M %d.%m')}")
        except Exception:
            pass
