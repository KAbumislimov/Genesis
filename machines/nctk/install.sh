#!/bin/bash
# Установка/восстановление nctk с нуля из этого репозитория.
...
cp -f "$REPO_DIR/scripts/campus-cron-landau-local.sh" "$HOME_DIR/"
...
chmod +x "$HOME_DIR/campus-cron-landau-local.sh" \
...
mkdir -p /var/lib/campus-player/inbox 2>/dev/null || sudo mkdir -p /var/lib/campus-player/inbox && sudo chown nctk:nctk /var/lib/campus-player/inbox
