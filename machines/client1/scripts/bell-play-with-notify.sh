...
/home/client1/cron_notify.sh start "$TIME" "$PEREMENA" "$TRACK" "$DURATION" "$VOL" "$DAY" 2>/dev/null || true

RC=0
if [[ -x /usr/local/bin/bell-play.sh ]]; then
    /usr/local/bin/bell-play.sh "$@" || RC=$?
elif [[ -x /home/client1/bell-play.sh ]]; then
    /home/client1/bell-play.sh "$@" || RC=$?
else
...
/home/client1/cron_notify.sh end "$(date +%H:%M)" "$PEREMENA" "$TRACK" "$DURATION" "$VOL" "$DAY" "$SUCCESS" 2>/dev/null || true
exit $RC
