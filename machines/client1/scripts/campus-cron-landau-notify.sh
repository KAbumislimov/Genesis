...
# Pauza yoxlanışı (~/.cron_paused bayrağı). FORCE_PLAY=1 (veb-ui-nin
# "⚡ sürətli işə salma" düyməsi) bu yoxlamanı bypass edir — insan
# əl ilə klik edibsə, pauzadan asılı olmayaraq işə düşməlidir.
...
if [[ -z "$FORCE_PLAY" ]] && [[ -f "$HOME/.cron_paused" ]]; then
